...
# vault write auth/userpass/users/chun password=thispasswordsucks policies=db-engineering,kv-user-template

tput clear
cyan "#-------------------------------------------------------------------------------
#--- Running: $0: Testing Engineering requirements
#-------------------------------------------------------------------------------\n"
echo
unset PGUSER PGPASSWORD
USERNAME=chun

green "Ensure the root token isn't being used"
...
# UserPass method is a backup for ldap issues.
...
echo
white "Getting dynamic db credentials from Vault. There are more elegant ways of 
doing this, but OSX doesn't support process substitution in its version of Bash, 
so working around (e.g. source <(cmd))"
...
pe "vault read -format=json db-blog/creds/mother-engineering-full-1h | \\
...
green "Setting the postgres environment variables to the dynamic creds, so we can run PSQL with those dynamic creds"
